...
class Coordinator:
    ...
    def on_edges(self) -> dict[str, dict]:
        """
        Get which edge each robot is currently on (if any).

        Args:
            kt: Current time step

        Returns:
            dict: {robot_id: {'on_edge': bool, 'edge': tuple or None}}
                  Example: {'robot_1': {'on_edge': True, 'edge': ((0,0), (10,0))}}
        """
        robots_on_edges = {}

        for rid in self.robot_ids:
            robot_planner = self.robot_manager.get_planner(rid)
            robot_position = self.robot_manager.get_robot_state(rid)
    
            target_node = robot_planner.current_target_node
            ref_path = robot_planner._ref_path
    
            # Get current edge
            target_idx = self.get_target_node_index(ref_path, target_node)
    
            if target_idx is None or target_idx == 0:
                robots_on_edges[rid] = {'on_edge': False, 'edge': None}
                continue
    
            prev_node = ref_path[target_idx - 1]
    
            # Check if robot is at either node
            if self._is_at_position(robot_position, prev_node, 0.1):
                robots_on_edges[rid] = {'on_edge': False, 'edge': None}
            elif self._is_at_position(robot_position, target_node, 0.1):
                robots_on_edges[rid] = {'on_edge': False, 'edge': None}
            else:
                # Robot is on the edge
                edge = (tuple(prev_node), tuple(target_node))
                robots_on_edges[rid] = {'on_edge': True, 'edge': edge}

        return robots_on_edges
    
    def _is_at_position(self, position1: list[float], position2: list[float], tolerance: float = 0.1) -> bool:
        """
