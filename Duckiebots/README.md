...
This project is based on [this template](https://github.com/duckietown/template-ros). 

## 1. Code Handeling
Code to run the bot can be found under packages ```my_packages/src``` for the duckiebots and for the MPC you can find it under ```mpc/src```.

NOTE: This project is a work in progress and is not done.

### Duckiebot Code 
As previously mention you can find the Duckiebot code under m̀y_packages/src and in that directory there is several packages that are there from the example projects on [this website](https://docs.duckietown.com/daffy/devmanual-software/beginner/ros/introduction-to-using-ros-with-duckietown.html). The ones that are changed are ```my_subscriber_node``` and ```twist_control_node```. These are the packages you want to launch to run the robots (more on how to do this in the Duckiebot Handeling Guide). Twist control will go on the pathing robots and my_subsrcriber will go on the watchtower robot.  
...
### Launch files
In ```launchers``` there is different launch files to run the code on the robots (more on how to do so in Duckiebot Handeling Guide). The ones that should be run as previously mentioned are ```twist-control``` and ```my-subscriber```. 


## 2. Duckiebot Handeling Guide 
